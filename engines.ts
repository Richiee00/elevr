...
// Todos los objetivos de Running requieren un Test VAM real: ya no se pide ningún tiempo de 10K/21K
// en el onboarding para calcular objetivos de carrera ni zonas de ritmo (podría estar desactualizado
// y no reflejar la forma física actual). El Test VAM se hace en Diagnóstico, ya con el plan
// generado, y es obligatorio completarlo antes de poder empezar a entrenar (ver App.tsx).
...
// Umbral de "compromiso temprano" que desbloquea el IMC, los objetivos de carrera y el predictor de
// marcas en Diagnóstico/Perfil: por credibilidad, ningún número calculado se muestra de primeras,
// solo una vez que el usuario ha demostrado cómo responde de verdad a sus primeros entrenamientos.
export const EARLY_ENGAGEMENT_THRESHOLD = 0.6;
export const EARLY_ENGAGEMENT_WEEKS = 2;

export function hasEarlyEngagement(plan: TrainingPlan, completedWorkouts: Record<string, WorkoutCompletionRecord>): boolean {
  const firstWeeks = plan.weeks.slice(0, EARLY_ENGAGEMENT_WEEKS);
  const activeSessions = firstWeeks.flatMap(w => w.sessions.filter(s => s.type !== "descanso"));
