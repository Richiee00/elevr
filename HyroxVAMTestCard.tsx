...
import { HyroxVAMTestResult } from "./hyroxTypes";
import { calculateVAMFromTest, VAM_TEST_DURATION_MIN } from "./hyroxEngine";
import { Check, X } from "lucide-react";

interface HyroxVAMTestCardProps {
  onComplete: (result: HyroxVAMTestResult) => void;
  onCancel: () => void;
}

export default function HyroxVAMTestCard({ onComplete, onCancel }: HyroxVAMTestCardProps) {
  ...
  const handleSubmit = () => {
    if (!canSubmit) return;
    const { vamKmH, vamPaceMinKm } = calculateVAMFromTest(meters);
  ...
  return (
    <div className="bg-zinc-50 border border-zinc-200/80 rounded-2xl p-5 space-y-4">
      <p className="text-xs text-zinc-600 font-medium leading-relaxed">
        Corre al máximo esfuerzo que puedas sostener durante <span className="font-bold text-zinc-900">{VAM_TEST_DURATION_MIN} minutos</span> (pista, cinta o
